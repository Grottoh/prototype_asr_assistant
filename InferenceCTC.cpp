...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    if (interactive) {
      LOG(INFO)
          << "Waiting the input in the format [audio_path].";
    ...
    if (audioPath == "") {
      LOG(INFO)
          << "Please provide non-empty input";
      continue;
    }
    ...
    /*
	for(int i=0; i<tokenPrediction.size(); ++i)
    	LOG(INFO) << ">>> " << tokenPrediction[i] << ' ';
    /**/
    
    /*
	LOG(INFO) << ">>> " << "XXXXXXXXXXXXX";
	LOG(INFO) << ">>> " << typeid(rawTokenPrediction).name() << ' ';
	LOG(INFO) << ">>> " << typeid(tokenPrediction).name() << ' ';
	LOG(INFO) << ">>> " << typeid(wordPrediction).name() << ' ';
	/**/  
	
    /*
	auto tokenPredictionStr = fl::lib::join("", tokenPrediction);
    LOG(INFO) << ">>> " << tokenPredictionStr;
	/**/
	
	
    std::stringstream tokonStream;
	for(int i=0; i<rawTokenPrediction.size(); ++i) {
		//if(i != 0)
			//tokonStream << " ";
    	tokonStream << tokenDict.getEntry(rawTokenPrediction[i]);
    }
    std::string tokonStr = tokonStream.str();
	/**/  
    LOG(INFO) << "--> " << tokonStr;
	
    LOG(INFO) << "Predicted output for "
              << audioPath << ":\n";
              
    LOG(INFO) << ">>> " << wordPredictionStr;
